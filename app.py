...
def cargar_datos():
    ...
    df_sintomas = pd.read_csv("data/sintomas_respiratorios_ninos.csv", sep=";", encoding="latin-1")
    return df_aire, df_estaciones, df_mort, df_sintomas

df_aire, df_estaciones, df_mort, df_sintomas = cargar_datos()
...
with tab1:
    ...
    st.info(
        "⚠️ Esta vista usa la serie histórica de estaciones (2010 - feb 2021) para el "
...
with tab2:
    ...
    st.caption(
        ...
        "frente a estaciones con más años de monitoreo."
    )
...
with tab3:
    ...
    with col1:
        ...
        fig3 = px.line(
            ...
            title="Tendencia anual de PM10 (2010-2021)",
            labels={"valor": "PM10 promedio anual (µg/m³)", "anio": "Año"},
...
with tab4:
    ...
    st.markdown("#### Respaldo: síntomas respiratorios en niños menores de 5 años")
    sintomas_por_localidad = (
        df_sintomas.groupby("Localidad")[["Casos de sibilancia último año", "Casos de ausentismo escolar"]]
        .sum()
        .reset_index()
        .sort_values("Casos de sibilancia último año", ascending=False)
        .head(10)
    )
    fig6 = px.bar(
        sintomas_por_localidad, x="Localidad",
        y=["Casos de sibilancia último año", "Casos de ausentismo escolar"],
        barmode="group",
        labels={"value": "Número de casos reportados", "variable": "Indicador"},
        height=350,
    )
    st.plotly_chart(fig6, use_container_width=True)
    st.caption(
        "Refuerza el argumento de salud pública de las zonas prioritarias con un indicador "
        "independiente: síntomas respiratorios y ausentismo escolar en menores de 5 años (SDS)."
    )
